...
Where $z$ is the number of iterations ran for. $\overline{d}$ is each agent’s average distance to their duplicated data’s data points. $\overline{q}$ is the average duplications over all data points, and $N$ is global current active agents. $\overline{ \varpi }$ is the mean instability over all data points. $\overline{\gamma}$ is the spread of agents used in memory space.
